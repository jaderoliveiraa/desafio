            ...
            $.ajax({url: base_url + "cursos/getJsonByCategory/" + $(this).attr("data-id"), type: "POST", data: {}, dataType: "json", success: function (response) {
                    ...
                    if (response.length > 0) {
                        $.each(response, function (key, value) {
                            $("#contentListCourses").append("<div class=\"col-lg-3 col-md-3 col-sm-6 col-xs-6\">" + "    <div class=\"col-item itemOfListAll\">" + "        <div class=\"photo\">" + "            <a href=\"" + value.link_informacoes + "\">" + "                <img src=\"" + value.imagem + "\" alt=\"\" class=\"img-responsive\"/>" + "            </a>" + "            <div class=\"cat_row text-right\">" + "                <span>" + "                    <i class=\"icon-clock\"></i>" + value.carga_horaria + "                </span>" + "            </div>" + "        </div>" + "        <div class=\"info\">" + "            <div class=\"row\">" + "                <div class=\"course_info col-lg-12 col-md-12 col-sm-12\">" + "                    <h4>" + value.nome + "</h4>" + "                </div>" + "            </div>" + "            <div class=\"separator clearfix\">" + "                <a href=\"" + value.link_informacoes + "\" title=\"Saiba mais sobre este curso\" class=\"btn btn-purple btn-block no-border-radius\">" + "                    Mais informações" + "                </a>" + "            </div>" + "        </div>" + "    </div>" + "</div>")
                        })
                    } else {
                        $("#contentListCourses").html("<h3>Oops, parece que não encontramos nenhum curso relacionado a esta categoria.</h3>")
                    }
                }, error: function () {
                    alert("Ocorreu um erro na solicitação, por favor tente novamente.")
                }, fail: function () {
                    alert("Ocorreu uma falha na solicitação, por favor verifique sua conexão e tente novamente.")
                }})
        })
    }
    ...
    if ($("#formCheckCertificate").length > 0) {
        ...
        $("#formCheckCertificate").on("submit", function () {
            ...
            if (form.find("input[name=code]").val() == "") {
                pnotify("Oops!!!", "você deve informar o número do seu certificado.", "error");
                return false
            }
            miniLoadIn();
            ...
            $.ajax({url: form.attr("action"), type: "POST", data: form.serialize(), dataType: "json", success: function (response) {
                    if (response.status == "success") {
                        ...
                        form.find(".alertMsg .col-md-12").html("" + "<div class=\"alert alert-danger no-border-radius\">" + "    <strong> <i class=\"fa fa-warning\"></i> Certificado não reconhecido </strong>" + "    <p></p>" + "    <p> O código deste certificado não foi indentificado! </p>" + "</div>");
                        form.find(".alertMsg").slideDown(200)
                    }
                    miniLoadOut()
                }}).fail(function () {
                pnotify("Problema", "Falha na comunicação com o servidor.");
                miniLoadOut()
            }).error(function () {
                pnotify("Problema", "Erro na requisição ao servidor.");
                miniLoadOut()
            });
            return false
        })
            ...
            if (valida_email(_this_.find("input[name=email]").val())) {
                $.ajax({url: _this_.attr("action"), type: _this_.attr("method"), data: _this_.serialize(), dataType: "json", success: function (response) {
                        if (response.success == "true") {
                            ...
                            _this_.find(".message").html("<span class=\"text-success hard\">Aguarde, você será redirecionado.</span>");
                            window.location.href = _this_.find("input[name=redirect]").val()
                        } else {
                            _this_.find(".message").html("<span class=\"text-danger hard\">E-mail ou senha inválido</span>")
                        }
                    }, error: function (response) {
                        _this_.find(".message").html("<span class=\"text-danger hard\">Ocorreu uma falha na solicitação.</span>")
                    }, fail: function (response) {
                        _this_.find(".message").html("<span class=\"text-danger hard\">Falha na comunicação com o servidor, por favor verifique sua conexão com a internet.</span>")
                    }})
            } else {
                _this_.find(".message").html("<span class=\"text-danger hard\">Informe um e-mail válido.</span>")
            }
        })
    }
                    ...
                    if (response.success == "true") {
                        ...
                        $.ajax({url: _this_.attr("action") + "?new_inscricao=true", type: "GET", data: {cid: _this_.find("input[name=id_curso]").val()}, dataType: "json", success: function (response) {
                                if (response.success == "true") {
                                    ...
                                    _this_.find(".message").html("<span class=\"text-warning hard\">Seu cadastro foi efetuado, porém por alguma razão não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.</span>")
                                }
                            }, error: function () {
                                _this_.find(".message").html("<span class=\"text-warning hard\">Seu cadastro foi efetuado, porém ocorreu um erro na solicitação. Por gentileza tente novamente, caso o problema persista entre em contato conosco através dos nossos canais de atendimento.</span>")
                            }, fail: function () {
                                _this_.find(".message").html("<span class=\"text-danger hard\">Falha na comunicação com o servidor. Por gentileza, verifique sua conexão com a internet.</span>")
                            }})
                    } else {
                        _this_.find(".message").html("<span class=\"text-danger hard\">" + response.message + "</span>")
                    }
                        ...
                        if (response.success == "true") {
                            $.ajax({url: _this_.find("input[name=address_ava]").val() + "/controle.php?new_inscricao=true", type: "GET", data: {cid: _this_.find("input[name=id_curso]").val()}, dataType: "json", success: function (response) {
                                    if (response.success == "true") {
                                        ...
                                        _this_.find(".message").html("<span class=\"text-warning hard\">Seu usuário foi identificado, porém não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.</span>")
                                    }
                                }, error: function () {
                                    _this_.find(".message").html("<span class=\"text-warning hard\">Seu usuário foi identificado, porém ocorreu um erro na solicitação. Por gentileza tente novamente, caso o problema persista entre em contato conosco através dos nossos canais de atendimento.</span>")
                                }, fail: function () {
                                    _this_.find(".message").html("<span class=\"text-danger hard\">Falha na comunicação com o servidor. Por gentileza, verifique sua conexão com a internet.</span>")
                                }})
                        } else {
            ...
            $.ajax({url: _this_.attr("data-action"), type: "GET", data: {palestra_id: _this_.attr("data-id")}, dataType: "json", success: function (response) {
                    if (response.success == "true") {
                        if (typeof response.carrinho !== "undefined") {
                            ...
                            pnotify("Parabéns", "Sua inscrição nesta palestra foi confirmada.", "success")
                        }
                    } else {
                        pnotify("Atenção", response.message, "error")
                    }
                }, error: function () {
                    pnotify("Problema na solicitação", "Por alguma razão não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.", "error")
                }, fail: function () {
        ...
        $("form#newsletter").on("submit", function (e) {
            ...
            if (!valida_email($(this).find("#email_newsletter").val())) {
                ...
                $.ajax({url: base_url + "newsletter", type: "POST", data: {email: $(this).find("#email_newsletter").val()}, dataType: "json", success: function (response) {
                        if (response.status == "success") {
                            $("#divnewsletter").html("<div id=\"message-newsletter\" class=\"show\"> Seu e-mail está em nossa lista de novidades. </div>")
                        } else {
    ...
    if ($("form#lembrar-senha").length > 0) {
        $("form#lembrar-senha").on("submit", function (e) {
            ...
            if (!valida_email($this.find("#emailforgot").val())) {
                $this.find(".message").html("<span class=\"text-danger\">Informe um e-mail válido.</span>").slideDown(200)
            } else {
                $.ajax({url: $this.attr("action"), type: "POST", data: {emailforgot: $(this).find("#emailforgot").val()}, dataType: "json", success: function (response) {
                        ...
                        $this.find(".message").html("<span class=\"text-danger\">Problema na solicitação</span>").slideDown(200)
                    }, fail: function () {
                        $this.find(".message").html("<span class=\"text-danger\">Ocorreu uma falha na solitação</span>").slideDown(200)
                    }})
            }
        })
    }
    ...
    if ($(".formAderirOferta").length > 0) {
        ...
        $(".formAderirOferta").on("submit", function (e) {
            ...
            if (!($("input[name='regulamento']").is(':checked'))) {
                $(".message").addClass("danger").html("É necessário aceitar o regulamento.").slideDown(200);
            ...
            if (!valida_email($(this).find("#email").val())) {
                $(".message").addClass("danger").html("Informe um e-mail válido.").slideDown(200);
            } else {
...
//Função pra desmarca e marcar checkbox simultaneos
