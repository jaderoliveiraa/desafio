...
function validateFormNewUser(e) {
    ...
    return (
        ...
        $("input[name=email]").val() != $("input[name=confEmail]").val() &&
            ($("input[name=email]").focus(), (a = !1), $("#formRegisterNewStudent").find("#message").addClass("alert alert-warning"), $("#formRegisterNewStudent").find("#message").html('<div class="alert alert-warning"><span class="text-danger">E-mails informados são diferentes!</span></div>')),
        ...
        $("input[name=email]").val() != $("input[name=confEmail]").val() &&
            ($("#formRegisterNewStudent").find(".message").html('<div class="alert alert-warning"><span class="text-danger">Os e-mails não conferem!</span></div>'), (a = !1)),
        $("input[name=senha]").val() != $("input[name=confSenha]").val() &&
            ($("#formRegisterNewStudent").find(".message").html('<div class="alert alert-warning"><span class="text-danger">As senhas não conferem!</span></div>'), (a = !1)),
        "number" != typeof parseInt($("input[name=nivel_perfil]").val()) &&
            ($("#formRegisterNewStudent").find(".message").html('<div class="alert alert-warning"><span class="text-danger">Selecione uma opção de em "você é:"</span></div>'), (a = !1)),
        a
    );
}
        ...
        $(".btn-planos-inscricao").click(function () {
            ...
            $(".btn-plano-vip").click(function () {
                ...
                $.ajax({
                    ...
                    success: function (e) {
                        "success" == e.status && (pnotify("Parabéns!!!", e.message, "success"), $(".form-dados-usuario")[0].reset());
                    },
                });
            }),
            ...
            $("#formCheckCertificate").length > 0 &&
                ...
                $("#formCheckCertificate").submit(function () {
                    ...
                    return "" == e.find("input[name=code]").val()
                        ? (pnotify("Oops!!!", "você deve informar o número do seu certificado.", "error"), !1)
                        ...
                        ? (miniLoadIn(),
                          ...
                          $.ajax({
                              ...
                              success: function (a) {
                                  "success" == a.status
                                      ...
                                      : (e
                                            .find(".alertMsg .col-md-12")
                                            .html(
                                                '<div class="alert alert-danger no-border-radius"><strong><i class="fa fa-warning"></i> Certificado não reconhecido</strong><br><p>O código deste certificado não foi identificado! </p></div>'
                                            ),
                                        e.find(".alertMsg").slideDown(200),
                                        miniLoadOut());
                              },
                              ...
                              .fail(function () {
                                  pnotify("Problema", "Falha na comunicação com o servidor."), miniLoadOut();
                              })
                              .error(function () {
                                  pnotify("Problema", "Erro na requisição ao servidor."), miniLoadOut();
                              }),miniLoadOut(),
                          !1)
                        : (miniLoadIn(),
                          e.find(".alertMsg").slideUp(200),
                ...
                $("#formLogin").submit(function (e) {
                    ...
                    valida_email(a.find("input[name=email]").val())
                        ? $.ajax({
                              ...
                              success: function (e) {
                                  if("true" == e.success){
                                      (a.find(".message").html('<span class="text-success hard">Aguarde, você será redirecionado.</span>'),
                                      ...
                                      if(e.message && e.message.length > 0){
                                        ...
                                        a.find(".message").html('<span class="text-danger hard">E-mail ou senha inválido</span>');
                                      }
                                      grecaptcha.reset(0);
                                    }
                              },
                              error: function () {
                                  a.find(".message").html('<span class="text-danger hard">Ocorreu uma falha na solicitação.</span>');
                              },
                              fail: function () {
                                  a.find(".message").html('<span class="text-danger hard">Falha na comunicação com o servidor, por favor verifique sua conexão com a internet.</span>');
                              },
                          })
                        : a.find(".message").html('<span class="text-danger hard">Informe um e-mail válido.</span>'),grecaptcha.reset(0);
                }),
                ...
                return (
                    ...
                    1 == validateFormNewUser("#formRegisterNewStudent") &&
                        $.ajax({
                            ...
                            error: function () {
                                grecaptcha.reset(), a.find("#message").html('<span class="text-danger hard">Ocorreu uma falha na solicitação.</span>');
                            },
                            fail: function () {
                                grecaptcha.reset(), a.find("#message").html('<span class="text-danger hard">Falha na comunicação com o servidor, por favor verifique sua conexão com a internet.</span>');
                            },
                        }),
            ...
            $(".formRegisterNewStudentInscricao").submit(function (e) {
                ...
                return (
                    ...
                    $.ajax({
                        ...
                        success: function (e) {
                            "true" == e.success
                                ? (dataLayer.push({ event: "user_register", customDataObject: { page: "PAGE: Informações de curso" } }),
                                  $.ajax({
                                      ...
                                      success: function (e) {
                                          "true" == e.success &&
                                              ...
                                              a.find("#message").html('<span class="hard">Aguarde, você será redirecionado.</span>'),
                                              void 0 !== e.link
                                                  ? (window.location.href = a.find("input[name=address_ava]").val() + "/" + e.link)
                                                  : (a.find("#message").addClass("alert alert-info"),
                                                    a.find("#message").html(
                                                            '<span class="text-warning hard">Seu cadastro foi efetuado, porém por alguma razão não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.</span>'
                                                        ),
                                                    a.find("#message").show(),
                                      ...
                                      error: function () {
                                          grecaptcha.reset(),
                                              a.find("#message").addClass("alert alert-info"),
                                              a.find("#message").html(
                                                      '<span class="hard">Seu cadastro foi efetuado, porém ocorreu um erro na solicitação. Por gentileza tente novamente, caso o problema persista entre em contato conosco através dos nossos canais de atendimento.</span>'
                                                  );
                                              a.find("#message").show();
                                      },
                                      fail: function () {
                                          grecaptcha.reset(),
                                              ...
                                              a.find("#message").html('<span class="hard">Falha na comunicação com o servidor. Por gentileza, verifique sua conexão com a internet.</span>'),
                                              a.find("#message").show();
                                      },
                                  }))
                        ...
                        fail: function () {
                            grecaptcha.reset(), a.find("#message").addClass("alert alert-warning"), a.find("#message").html('<span class="hard">Falha na comunicação com o servidor, por favor verifique sua conexão com a internet.</span>');
                            a.find("#message").show();
                        },
                    }),
                    !1
                                ...
                                if("true" == e.success){
                                  $.ajax({
                                              ...
                                              console.log("true?");
                                                "true" == e.success
                                                    ? (ga("send", "event", "inscricao " + n.find("input[name=plano]").val(), "fazer inscricao", "insc_info_" + n.find("input[name=id_curso]").val()),
                                                      n.find("#message").html('<span class="text-success hard">Aguarde, você será redirecionado.</span>'),
                                                    ...
                                                    : n.find("#message").addClass("alert alert-warning").html('<span class="text-warning hard">Seu usuário foi identificado, porém não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.</span>').show();
                                            },
                                            error: function () {
                                                n.find("#message").addClass("alert alert-warning").html(
                                                    '<span class="text-warning hard">Seu usuário foi identificado, porém ocorreu um erro na solicitação. Por gentileza tente novamente, caso o problema persista entre em contato conosco através dos nossos canais de atendimento.</span>'
                                                ).show();
                                            },
                                            fail: function () {
                                                n.find("#message").addClass("alert alert-warning").html('<span class="text-danger hard">Falha na comunicação com o servidor. Por gentileza, verifique sua conexão com a internet.</span>').show();
                                            },
                                        });
                                      }else{
                                        grecaptcha.reset(0);
                                        ...
                                        if(e.message && e.message.length >0){
                                          ...
                                          n.find("#message").addClass("alert alert-warning").html('<span class="text-danger hard">E-mail ou senha inválido</span>').show();
                                        }
                                        return;
                                    }
                              },
                              error: function () {
                                  n.find("#message").addClass("alert alert-warning").html('<span class="text-danger hard">Ocorreu uma falha na solicitação, verifique os dados ou tente novamente mais tarde.</span>').show();
                              },
                              fail: function () {
                                  n.find("#message").addClass("alert alert-warning").html('<span class="text-danger hard">Falha na comunicação com o servidor, por favor verifique sua conexão com a internet.</span>').show();
                              },
                          }))
                        : n.find("#message").html('<span class="text-danger hard">Informe um e-mail válido.</span>').show(),
                    !1
                );
            }),
            $("*[data-process-incricao=true]").length > 0 &&
                $("*[data-process-incricao=true]").click(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (a) {
                            "true" == a.success
                                ? (pnotify("Sucesso", "Aguarde, você será redirecionado.", "success"),
                                ...
                                : pnotify("Erro", "Por alguma razão não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.", "error");
                        },
                        error: function () {
                            pnotify("Problema na solicitação", "Por alguma razão não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.", "error");
                        },
                        fail: function () {
                            pnotify("Falha na solicitação", "Falha na comunicação com o servidor. Por gentileza, verifique sua conexão com a internet.", "warning");
                        },
                    });
                }),
            $("*[data-process-palestra=true]").length > 0 &&
                $("*[data-process-palestra=true]").click(function () {
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        success: function (e) {
                            "true" == e.success ? (void 0 !== e.carrinho ? pnotify("", "Aguarde...", "success") : pnotify("Parabéns", "Sua inscrição nesta palestra foi confirmada.", "success")) : pnotify("Atenção", e.message, "error");
                        },
                        error: function () {
                            pnotify("Problema na solicitação", "Por alguma razão não foi possível concluir sua inscrição. Por favor atualize sua página e tente novamente.", "error");
                        },
                    ...
                    e.preventDefault(),
                        ...
                        valida_email($(this).find("#email_newsletter").val())
                            ? $.ajax({
                                  ...
                                  success: function (e) {
                                      "success" == e.status
                                          ? $("#divnewsletter").html('<div id="message-newsletter" class="show"> Seu e-mail estÃ¡ em nossa lista de novidades. </div>')
                                          : $("#message-newsletter").addClass("danger").html(e.message).slideDown(200);
                                  },
                                  error: function () {
                                      $("#message-newsletter").addClass("danger").html("Problema na solicitação").slideDown(200);
                                  },
                                  fail: function () {
                                      $("#message-newsletter").addClass("danger").html("Ocorreu uma falha na solitação.").slideDown(200);
                                  },
                              })
                    ...
                    a.find(".message").slideUp(200).html(""),
                        valida_email(a.find("#emailforgot").val())
                            ? $.ajax({
                                  ...
                                  success: function (e) {
                                      "true" == e.success
                                          ? a.find(".message").html('<span class="text-success" style="font-weight: 800;">Enviamos o link para cadastrar uma nova senha no seu email. Por favor, verifique sua caixa de entrada bem como o lixo eletrônico.</span>').slideDown(200)
                                          : a
                                                .find(".message")
                                  ...
                                  error: function () {
                                      a.find(".message").html('<span class="text-danger" style="font-weight: 800;">Problema na solicitação</span>').slideDown(200);
                                  },
                                  fail: function () {
                                      a.find(".message").html('<span class="text-danger" style="font-weight: 800;">Ocorreu uma falha na solitação</span>').slideDown(200);
                                  },
                              })
                            : a.find(".message").html('<span class="text-danger" style="font-weight: 800;">Informe um e-mail válido.</span>').slideDown(200);
                }),
                        ...
                        ? ($(".message").hide(),
                          ...
                          valida_email($(this).find("#email").val())
                              ? ($.post(base_url + "/ava/controle.php?aderirOferta=true", $(this).serialize(), function (e) {
                                ...
                                !1)
                              : void $(".message").addClass("danger").html("Informe um e-mail válido.").slideDown(200))
                        : ($(".message").addClass("danger").html("É necessário aceitar o regulamento.").slideDown(200), $(".formAderirOferta > button").attr("disabled", "disabled"), !1)
                );
            }),
...
$(".participar").on("click", function () {
    ...
    valida_email(e.find("input[name=email]").val())
        ? $.ajax({
              ...
              error: function () {
                  pnotify("Problema na solicitação", "Por alguma razão não foi possível concluir essa ação. Por favor atualize sua página e tente novamente.", "error");
              },
              fail: function () {
                  pnotify("Falha na solicitação", "Falha na comunicação com o servidor. Por gentileza, verifique sua conexão com a internet.", "warning");
              },
          })
        : pnotify("Dado inválido", "Email inválido. Por favor insira um e-mail válido.", "error");
});
...
$("form#generic-post").on("submit", function (e) {
    ...
    a.find(".message").slideUp(200).html(""),
        $.ajax({
                  ...
                  success: function (e) {
                      if("true" == e.success){
                        ...
                        if (typeof e.url_redirect !== 'undefined' && e.url_redirect) {
                          a.find(".message").html('<span class="text-success" style="font-weight: 800;">Você será redirecionado em 5 segundos...</span>').slideDown(200);
                          if (typeof pnotify !== 'undefined') {pnotify("Sucesso!","Você será redirecionado em 5 segundos...","success")};
                  ...
                  error: function () {
                      a.find(".message").html('<span class="text-danger" style="font-weight: 800;">Problema na solicitação, atualize a página e tente novamente!</span>').slideDown(200);
                      if (typeof pnotify !== 'undefined') {pnotify("Erro!","Problema na solicitação, atualize a página e tente novamente!","error")};
                  },
                  fail: function () {
                      a.find(".message").html('<span class="text-danger" style="font-weight: 800;">Ocorreu uma falha na solitação, atualize a página e tente novamente!</span>').slideDown(200);
                      if (typeof pnotify !== 'undefined') {pnotify("Erro!","Ocorreu uma falha na solitação, atualize a página e tente novamente!","error")};
                  },
              });
});
